...
/* types */
typedef	uint32_t	fat_add_t;    /* cluster adresse */
...
struct fat_bs{
	...
	u8    bs_ldnr;         		/* Logical Drive No (BIOS INT 13h) */
	u8    bs_headnr;          /* Head No */
	u8    bs_eboot;     		  /* Extended Boot Signature, Always 0x29, to identify if the next three values are valid */
	u32   bs_dserial;         /* Serial No of the Device */
	u8  	bs_pname[11];       /* Partition Name (ASCII) */
	u8  	bs_ptype[8];        /* FAT type (ASCII) */
	u8    bs_bootcode[448];  	/* Executable Boot Code */
	u16   bs_sig;          		/* Executable Signature (0xAA55) */
}__attribute__ ((__packed__));
...
/* FAT12/16 Infosector */
struct fat32_info{
	u32   i_m1;          /* FAT32 info sector magic value 1 (0x41615252) */
	u8    i_res1[480];   /* Reserved */
	u32   i_m2;          /* FAT32 info sector magic value 2 (0x61417272) */
	u32   i_fclu;        /* Number of free clusters */
	u32   i_nfclu;       /* Next free cluster */
	u8    i_res2[12];    /* Reserved */
	u32   i_sig;         /* Executable Signature (0xAA550000) */
}__attribute__ ((__packed__));
