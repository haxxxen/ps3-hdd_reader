...
int ufs_read_data(HANDLE device, struct fs *fs, struct ufs2_dinode *di, ufs2_block_list *block_list,
...
HANDLE get_file_handle()
{
	...
	device = CreateFile("backup.bin", GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, 0, 0);
...
int get_partitions(HANDLE device)
{
	...
	for(i = 0; i < p_count; i++){
		...
		if(vflash_m1 == MAGIC1 && vflash_m2 == MAGIC2){
			...
			flash_start = _ES64(vflash_pt->d_partitions[1].p_start) + vflash_start;	  /* vflash 2(FAT16) */
			flash_size = _ES64(vflash_pt->d_partitions[1].p_size);
			flash2_start = _ES64(vflash_pt->d_partitions[2].p_start) + vflash_start;	/* vflash 3(FAT16) */
			flash2_size = _ES64(vflash_pt->d_partitions[2].p_size);
			flash3_start = _ES64(vflash_pt->d_partitions[3].p_start) + vflash_start;	/* vflash 4(FAT12) */
			flash3_size = _ES64(vflash_pt->d_partitions[3].p_size);
		}
	}
...
s64 read_device(HANDLE device, u8 *buf, u64 numbytes, u64 dev_off)
{
	...
	if(numbytes > sec_rest){															/* wenn noch mehr kopiert werden muss... */
		...
		if((remain / SECTOR_SIZE) > 0){											/* wenn rest byte >= 512... */
			...
			ReadFile(device, tmp2, r_sec * SECTOR_SIZE, &read, NULL);			/* alle ganzen sektoren einlesen... */
...
* 	ufs_inop num_blocks					= 
***********************************************************************/
int ufs_read_data(HANDLE device, struct fs *ufs2, struct ufs2_dinode *di, ufs2_block_list *block_list,
		u8 *buf, ufs_inop start_block, ufs_inop num_blocks)
{
	int i, bsize;
...
void usage(){
	...
	printf("               backup must called \"backup.bin\" and be in program-folder.\n");
...
int main(int argc, char* argv[])
{
	...
	HANDLE device = NULL;								/* devive handle */
  ...
  //_print_buf(encdec_key2, 0, 2);
	...
	/* get handle (file/hdd), and get ps3 type(fat/slim) */
	if(argv[1]){
	...
	if(argc == 5){
		if(strcmp(argv[2], "dev_hdd0") == 0 && hdd0_start != 0){												/* if partition dev_hdd0... */
			ufs2 = ufs2_init(device);																								      /* init gameOS... */
			if(ufs2 == NULL){
				if(strcmp(argv[2], "print") == 0){
			printf("print....\n");
		}
		...
		else if(strcmp(argv[2], "dev_hdd1") == 0){																			/* if partition dev_hdd1... */
		...
		else if(strcmp(argv[2], "dev_flash") == 0){																			 /* if partition dev_flash... */
